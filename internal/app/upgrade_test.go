...
// TestRunArgs_UpgradeDryRun verifies that `conpas-ai upgrade --dry-run` runs without
...
// TestRunArgs_UpgradeNoArgs runs `conpas-ai upgrade` without flags.
...
// TestRunArgs_UpgradeToolFilter verifies that `conpas-ai upgrade engram` filters
...
func TestRunArgs_UpgradeToolFilter(t *testing.T) {
	...
	if strings.Contains(out, "conpas-ai") && !strings.Contains(out, "engram") {
...
func TestRenderUpgradeReport_PerToolSemantics_Deterministic(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantContains:   []string{"conpas-ai", "[--]", "source build"},
			...
			results: []upgrade.ToolUpgradeResult{
				{
					...
					NewVersion: "1.5.0",
					Status:     upgrade.UpgradeSkipped,
					ManualHint: "Download from https://github.com/Thrasno/conpas-ai/releases",
				},
			},
			wantContains:   []string{"conpas-ai", "manual update required", "github.com", "[--]"},
			wantNotContain: []string{"[!!]", "FAILED"},
		},
		{
		...
		},
		{
			...
			wantContains:   []string{"engram", "[ok]", "conpas-ai", "[--]", "gga", "1 succeeded", "2 skipped"},
			wantNotContain: []string{"FAILED", "[!!]"},
		},
	}
